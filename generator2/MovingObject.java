...
private boolean computeNextPoint (int newTime, Reporter reporter) {
	...
	if (actDist == 0)	// in the case of identical nodes
		actDist = 1;
	double actWeight = actEdge.getWeight();//时间吗？
	double speed = actDist/actWeight;//速度
	...
	while (true) {
		...
		if (relDist+maxDistOnEdge < actDist) {
			...
			Node nextNode = route.getDestinationNode();
 			repNum = reporter.reportMovingObjectAndEdge(newTime,id,repNum,objClass,lastNode.getID(),relDist,actEdge,lastX,lastY,speed,doneDist,nextNode.getX(),nextNode.getY(),objClasses.getReportProbability(objClass));
...
public void reportEnd (Reporter reporter) {
	...
	if (reporter != null) {
 		...
 		reporter.reportDisappearingObjectAndEdge(arrivalTime,id,repNum,objClass,lastNode.getID(),relDist,route.getEdge(),dest.getX(),dest.getY(),doneDist,objClasses.getReportProbability(objClass));
 		//repNum = reporter.reportMovingObjectAndEdge(newTime,id,repNum,objClass,relDist,actEdge,lastX,lastY,speed,doneDist,nextNode.getX(),nextNode.getY(),objClasses.getReportProbability(objClass));
...
public void reportNewObject (Reporter reporter) {
	...
	if (reporter != null) {
		...
		repNum = reporter.reportNewMovingObjectAndEdge (startTime,id,objClass,currEdge,start.getX(),start.getY(),speed,nextNode.getX(),nextNode.getY(),objClasses.getReportProbability(objClass));
