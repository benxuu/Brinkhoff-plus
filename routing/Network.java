 ...
 * Die Berechnung wird nur vom Startknoten aus durchgef黨rt.
 * @return berechneter Weg
 * @param start Startknoten
 * @param stop Stopknoten
 */
...
public PathEdge computeFastWay2 (Node start, Node stop) {
	...
	while (elementFound) {
		...
		// Fall 1: Knoten geh鰎t zu WAY1
		if (actNode.isMarked(WAY1)) {
			actNode.mark(FINAL);
			// falls er auch zu WAY2 geh鰎t, sind beide Wege nun aufeinander getroffen => fertig
			if (actNode.isMarked(WAY2))
				break;
			enlargeBorder (border, actNode,stop, WAY1, true);
		}	
		// Fall 2: Knoten geh鰎t zu WAY2
 ...
 * schlechter sind als der k黵zeste Weg. <BR>
 * Einschr鋘kungen: <BR>
 * - keine Zyklen <BR>
 * - keine R點kwege (kann evtl. auch ung黱stige Querverbindungen treffen <BR>
 * @return berechneter Weg
 * @param start Startknoten
 * @param stop Stopknoten
...
public PathEdge computeFastWays (Node start, Node stop, int threshold) {
	// Pr黤en, ob Weg berechenbar
	if ((start == null) || (stop == null) || (start.equals(stop)))
		return null;
	// Initialisierung
	border.reset();					// Rand
	PathEdge resPath = null;					// Ergebnis
	double shortestDistance = maxDistance;		// Distanz des k黵zesten Wegs
	...
	for (int numOfLoops=1; true; numOfLoops++) {
		...
		// Abarbeiten und Erg鋘zen des Randes
		boolean elementFound = border.fetchFirst();
		while (elementFound) {
			// Pr黤en, ob Weg zu lang => ggf. Durchlauf abbrechen
			if (border.returnDistance()*100/(100+threshold) > shortestDistance)
				break;
			Node actNode = border.returnNode();
			actNode.mark(FINAL);
			// Fall 1: Knoten geh鰎t zu NWAY1
			if (actNode.isMarked(WAY1)) {
				...
				// Fall 1b: Knoten geh鰎t auch zu NWAY2
				else if (actNode.isMarked(WAY2)) {
...
/**
 * Berechnung der Verbindung zwischen zwei BEST-Knoten, die 黚ergebenene Kante umfa遲.
 * Die Entfernung zum Zielknoten wird in den Rand-Elementen abgelegt.
 * @param border Rand gem溥 Dijkstra als Heap
 ...
 * @param threshold Prozentsatz, um den Wege l鋘ger sein d黵fen als der k黵zester Weg
 * @param shortestDistance L鋘ge des k黵zesten Weges
...
protected void computeNewConnection (BorderHeap border, Node start, Edge actEdge, Node stop, int wayMark, int threshold, double shortestDistance, PathEdge resPath) {
	...
	// Endpunkt der Verbindung feststellen und die Wegl鋘ge dahin berechnen
	long addDistance = 0;
	...
	// Falls beide zuvor BEST-Knoten, pr黤en ob Verbindung in Resultat vorliegt
	if (bothBest) {
		bothBest = (resPath.findEdge (actEdge) != null);
		if (!bothBest) {
			System.out.print("bothBest: "); searchNode.debugPrint(way); System.out.print(" -> "); oppositeNode.debugPrint(way); System.out.print(" bislang nicht in L鰏ung."); System.out.println();
		}
	}	

	// Falls man 黚er die Verbindung vorangekommen ist und den Schwellenwert nicht 黚erschreitet: Pfad als Ergebnis aufnehmen
 ...
 * @param way Index des gew黱schten Weges
 ...
 * Ggf. wird neben dem NWAY1-Weg auch der NWAY2-Weg hinzugef黦t.
...
protected PathEdge computeResultPath (Node firstNode, boolean computeNWAY2) {
	...
	// Ggf. Ergebnis zur點kgeben	
	if (!computeNWAY2) {
		return path;
	}
		
	// Berechnung NWAY2
	// Fall ber點ksichtigen, da�es keine NWAY2-Kante gibt, 
	if (firstNode.getWayEdge(NWAY2) == null) {
		return path;
	}
	// In Abh鋘gigkeit vom NWAY1-Weg Resultat verketten
...
System.err.println("Ich glaube das nachfolgende ist falsch, da sich path nicht 鋘dert");
 ...
 * @param nodeIn DataInputStream f黵 Knoten
 * @param edgeIn DataInputStream f黵 Kanten
 * @param objects Container f黵 drawable objects
 */
public void createByNetworkFiles (DataInputStream nodeIn, DataInputStream edgeIn, DrawableObjects objects) {
	...
	while ((actNode = nodes.read(nodeIn)) != null) {
		if (objects != null) {
			...
			if(point<1){	System.out.println(actNode.getID()+" "+actNode.getX()+" "+actNode.getY()+" "+actNode.getName());
			//actNode.
			}
		}
		point++;
	...
	while (!eof) {
		try {
			...
			if (actEdge != null) {
				...
				//	System.out.println(actEdge.getID()+" "+actEdge.getNode1()+" "+actEdge.getNode2()+" "+actEdge.getLength()+" "+actEdge.getWeight()+" "+actEdge.getName());
				if(line<1){ 
				//	System.out.println(actEdge.getID()+" "+actEdge.node1.getID()+" "+actEdge.node2.getID()+" "+actEdge.getLength()+" "+actEdge.getWeight()+" "+actEdge.getNode1().getID());
					System.out.println(actEdge.getID()+" "+actEdge.node1.getID()+" "+actEdge.node2.getID()+" "+actEdge.getLength());
				}
				}
			}
			else
 ...
 * Diese Version der Methode unterst黷zt die Berechnung von Verbindungen,
 * f黵 den Fall, da�die Erweiterung auf einen FINAL- oder BEST-Knoten trifft.
 * Die Entfernung zum Zielknoten wird grunds鋞zlich in den Rand-Elementen abgelegt.
 * @param border Rand gem溥 Dijkstra als Heap
...
protected void enlargeBorder (BorderHeap border, Node start, Node stop, int wayMark, int threshold, double shortestDistance, PathEdge resPath) {
	// Vorbereitungen
	int way = wayMark / WAY1;
	// 黚er alle Kanten des aktuellen Knotens iterieren
	Edge actEdge = start.getFirstEdge();
	while (actEdge != null) {
		// Ausschlie遝n, da�Zur點kgelaufen wird
		if (actEdge != start.getWayEdge(way)) {
			...
			if ((!oppositeNode.isMarked(FINAL)) &&
				...
				// oder Gewicht anpassen (bei akt. Heap-Implementierung ist dazu nichts n鰐ig)
				else {
					//System.out.print("ge鋘dert im Heap: "); oppositeNode.debugPrint(way); System.out.println();
					// Pr黤en, ob 黚er alten Weg neue Verbindung anlegbar ist
			...
			// ansonsten, Pr黤en, ob eine neue Verbindung anlegbar ist
 ...
 * @param considerDistToDest Soll die Entfernung zum Zielknoten ber點ksichtigt werden?
 */
